...
> Generated by release-please, which stopped running after 0.9.0 (2024-03-08);
...
* **`--small-thresh` is honoured again, and no longer silently overrides
  `--resolution`.** Cells below a hard-coded 100-record threshold were resolved
  by the tiny-cell fast path, which implements `cr-like` winner-take-all
  semantics. That happened *before* the resolution strategy was consulted, so
  `-r parsimony-em` quietly produced `cr-like` output for those cells and their
  gene-ambiguous UMIs were discarded rather than spread by EM. On a 73k-cell
  PBMC run this changed 81.5% of cells (0.53% of total UMI mass) and left 132
  cells at exactly zero that EM resolves to non-zero.

  The optimization stays on by default: cells this small carry on the order of
  one or two UMIs, all ambiguous between paralogous genes, and are filtered by
  any QC regardless of how they resolve. What was wrong is that the
  substitution was invisible and could not be turned off. `--small-thresh` had
  been parsed into `QuantOpts` and then dropped (`let _small_thresh = ...`)
  since it was introduced; it now controls the cutoff, and `--small-thresh 0`
  resolves every cell with the requested strategy.

* **The prefer-ambiguity splicing model no longer takes the fast path.** It
  resolves splicing-ambiguous UMIs differently from `extract_counts`, so it now
  falls through to the general path instead of having a policy substituted for
  it.

### Changed

* **`--small-thresh` default moves 10 -> 100** to match the constant it
  replaces, and the option is no longer hidden. The old default of 10 was
  already being written into `quant.json`, so the recorded provenance
  disagreed with what the code actually did.

* **`quant.json` records the tiny-cell path**: `num_tiny_cell_resolved` and
  `tiny_cell_resolved_cell_numbers`, alongside a log line naming the count, the
  threshold, and the strategy that was superseded. `alt_resolved_cell_numbers`
  does not cover these -- it tracks PUG falling back on oversized graphs, a
  different mechanism.
