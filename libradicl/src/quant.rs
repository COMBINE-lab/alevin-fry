...
struct VeloQuantOutputInfo {
    barcode_file: BufWriter<fs::File>,
    spliced_eds_file: BufWriter<GzEncoder<fs::File>>,
    unspliced_eds_file: BufWriter<GzEncoder<fs::File>>,
    feature_file: BufWriter<fs::File>,
...
fn velo_write_eqc_counts(
    ...
    // write the sets of genes that define each eqc
    let gn_eq_path = output_path.join("gene_eqclass.txt.gz");
    let gt_eq_path = output_path.join("gene_type_eqclass.txt.gz");
    let mut gn_eq_writer = BufWriter::new(GzEncoder::new(
        fs::File::create(gn_eq_path).unwrap(),
        Compression::default(),
    ));
    let mut gt_eq_writer = BufWriter::new(GzEncoder::new(
        fs::File::create(gt_eq_path).unwrap(),
        Compression::default(),
    ));
    ...
    // g_1 g_2 ... g_k eqid
    for (gene_list, eqid) in geqmap.global_eqc.iter() {
        for g in gene_list.0.iter().zip(gene_list.1.iter()) {
            gn_eq_writer
                .write_all(format!("{}\t", g.0).as_bytes())
                .expect("could not write to gene_eqclass.txt.gz");
            gt_eq_writer
                .write_all(format!("{}\t", g.1).as_bytes())
                .expect("could not write to gene_type_eqclass.txt.gz");
        }
        gn_eq_writer
            .write_all(format!("{}\n", eqid).as_bytes())
...
pub fn velo_quantify(
    ...
    let spliced_trimat = sprs::TriMatI::<f32, u32>::with_capacity(
        (hdr.num_chunks as usize, num_genes as usize),
        spliced_tmcap,
    );

    let unspliced_tmcap = if use_mtx {
        (0.2f64 * num_genes as f64 * hdr.num_chunks as f64).round() as usize
    } else {
        0usize
    };

    let unspliced_trimat = sprs::TriMatI::<f32, u32>::with_capacity(
        (hdr.num_chunks as usize, num_genes as usize),
        unspliced_tmcap,
    );

    let bc_writer = Arc::new(Mutex::new(VeloQuantOutputInfo {
        barcode_file: BufWriter::new(bc_file),
        spliced_eds_file: BufWriter::new(spliced_buffered),
        unspliced_eds_file: BufWriter::new(unspliced_buffered),
        feature_file: BufWriter::new(ff_file),
        spliced_trimat,
        unspliced_trimat,
        row_index: 0usize,
            ...
            while cells_remaining.load(Ordering::SeqCst) > 0 {
                if let Ok((cell_num, _nbyte, nrec, buf)) = in_q.pop() {
                    ...
                    for gid in 0..num_genes {
                        ...
                        if ct > 0.0 {
                            num_expr += 1;
                            expressed_vec.push((counts.spliced[gid], counts.unspliced[gid]));
                            expressed_ind.push(gid);
                        }
                    }
                    ...
                    let row_index: usize; // the index for this row (cell)
                    {
                        ...
                        //     // flatten the bootstraps
                        //     if summary_stat {
                        //         spliced_eds_mean_bytes = sce::eds::as_bytes(&bootstraps[0].spliced, num_genes)
                        //             .expect("can't convert vector to eds");
                        //         unspliced_eds_mean_bytes = sce::eds::as_bytes(&bootstraps[0].unspliced, num_genes)
                        //             .expect("can't convert vector to eds");
                        //         spliced_eds_var_bytes = sce::eds::as_bytes(&bootstraps[1].spliced, num_genes)
                        //             .expect("can't convert vector to eds");
                        ...
                        //                     .expect("can't convert vector to eds");
                        //             unspliced_bt_eds_bytes.append(&mut unspliced_bt_eds_bytes_slice);
                        //         }
                        //     }
                        // }
                        ...
                        if !use_mtx {
                            ...
                            writer
                                .spliced_eds_file
                                .write_all(&spliced_eds_bytes)
                                .expect("can't write to spliced matrix file.");
                            // write in eds format
                            writer
                                .unspliced_eds_file
                                .write_all(&unspliced_eds_bytes)
                                .expect("can't write to unspliced matrix file.");
                        } else {
                        ...
                        //         {
                        //             spliced_meanf
                        //                 .write_all(&spliced_eds_mean_bytes)
                        //                 .expect("can't write to spliced bootstrap mean file.");
                        //             unspliced_meanf
                        //                 .write_all(&unspliced_eds_mean_bytes)
                        //                 .expect("can't write to unspliced bootstrap mean file.");
                        //             spliced_varf.write_all(&spliced_eds_var_bytes)
                        //                 .expect("can't write to spliced bootstrap var file.");
                        //             unspliced_varf.write_all(&unspliced_eds_var_bytes)
                        //                 .expect("can't write to unspliced bootstrap var file.");
                        //         }
                        //     } else if let Some((spliced_bsfile, unspliced_bsfile)) = &mut writer.bootstrap_helper.bsfile {
                        //         spliced_bsfile
                        //             .write_all(&spliced_bt_eds_bytes)
                        //             .expect("can't write to spliced bootstrap file");
                        //         unspliced_bsfile
                        //             .write_all(&unspliced_bt_eds_bytes)
                        //             .expect("can't write to unspliced bootstrap file");
                        //     }
                        // }
                    }
    ...
    if use_mtx {
        let writer_deref = bc_writer.lock();
        let writer = &mut *writer_deref.unwrap();
        writer.spliced_eds_file.flush().unwrap();
        writer.unspliced_eds_file.flush().unwrap();
        // now remove it
        fs::remove_file(&spliced_mat_path)?;
        fs::remove_file(&unspliced_mat_path)?;
        let spliced_mtx_path = output_matrix_path.join("spliced_quants_mat.mtx");
        let unspliced_mtx_path = output_matrix_path.join("unspliced_quants_mat.mtx");
        sprs::io::write_matrix_market(&spliced_mtx_path, &writer.spliced_trimat)?;
        sprs::io::write_matrix_market(&unspliced_mtx_path, &writer.unspliced_trimat)?;
    }
    ...
    if dump_eq {
        velo_write_eqc_counts(&eqid_map_lock, num_genes, &output_path, &log);
    }
