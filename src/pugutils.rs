...
fn collapse_vertices_weighted(
    ...
    for (tx_index, txp) in eqmap.refs_for_eqc(vert.0).iter().enumerate() {
        ...
        let prob_vec = eqmap.probs_for_node(vert.0, vert.1, tx_index);
        ...
        while let Some(cv) = bfs_list.pop_front() {
            ...
            for nv in g.neighbors_directed(g.from_index(cv as usize), Outgoing) {
                ...
                if let Ok(_n) = n_labels.binary_search(txp) {
                    ...
                    //obtain the average probabilities for this UMI
                    let prob_vec = eqmap.probs_for_node(nv.0, nv.1, tx_index);
                    let avg_prob = prob_vec.iter().sum::<f64>() / prob_vec.len() as f64;
                    current_prob.push(avg_prob);
                }
            }
        ...
        if comp_verts.len() > 1 {
            ...
            while !uncovered_vertices.is_empty() {
                ...
                for v in uncovered_vertices.iter() {
                    ...
                    let (cand_mcc, cand_txp, cand_prob, eq_txps_prob) = if *is_long {
                        let (mcc, txp, prob, txp_probs) =
                            collapse_vertices_weighted(*v, &uncovered_vertices, g, eqmap, hasher_state);
                        (mcc, txp, prob, txp_probs)
                    } else {
                        let (mcc, txp) =
                    ...
                    if *is_long {
                        if best_mcc_prob < cand_prob {
                            ...
                            best_mcc_txp_probs = eq_txps_prob;
                        }
                    } else {
                ...
                let counter = gene_eqclass_hash.entry(global_genes.clone()).or_insert(0);
                *counter += 1;

                if *is_long {
                    let (count, prob_vec) = gene_eqclass_prob_hash.entry(global_genes).or_insert((0, vec![vec![]]));
                    *count += 1;
            ...
            if *is_long {
                if tl.len() == 1 {
                    ...
                    for (i, t) in tl.iter().enumerate(){
                        let prob_vec = eqmap.probs_for_node(g.from_index(*tv as usize).0, g.from_index(*tv as usize).1, i);
                        let avg_prob = prob_vec.iter().sum::<f64>() / prob_vec.len() as f64;
